...
const { openaiCreateMock, openaiOptions } = vi.hoisted(() => ({
...
vi.mock('openai', () => {
  class OpenAIMock {
    constructor(options: Record<string, unknown>) {
      Object.assign(openaiOptions, options);
    }
...
describe('shared AI client configuration', () => {
  ...
  it('returns a sanitized 503 when no AI provider key is configured', async () => {
    const key = process.env.OPENAI_API_KEY;
    const routerKey = process.env.OPENROUTER_API_KEY;
    delete process.env.OPENAI_API_KEY;
    delete process.env.OPENROUTER_API_KEY;
    const req = mockReq({
      id: 'request-missing-key',
      body: { content: 'Existing draft', instruction: 'Make it concise' },
    });
    const res = mockRes();
  ...
  it('sanitizes provider failures and returns a traceable 502', async () => {
    const req = mockReq({
    ...
    openaiCreateMock.mockRejectedValue(
      new Error('provider detail containing sk-secret-value')
    );
...
describe('rewriteDraft (controller)', () => {
  it('returns 200 with a non-empty draft string for valid content and instruction', async () => {
    const req = mockReq({
      body: {
        content: 'This is my current draft paragraph.',
    ...
    openaiCreateMock.mockResolvedValue({
      choices: [
        { message: { content: 'Rewritten and improved draft content' } },
      ],
    });
  ...
  it('returns 400 with field error on content when content is an empty string', async () => {
    const req = mockReq({
      body: { content: '', instruction: 'Make it more formal' },
    });
    const res = mockRes();
  ...
  it('returns 400 with field error on instruction when instruction is an empty string', async () => {
    const req = mockReq({
      body: { content: 'Some draft content here', instruction: '' },
    });
    const res = mockRes();
...
describe('generateCompanyResearch (controller)', () => {
  it('returns 200 with a non-empty draft string for a valid job owned by the user', async () => {
    const req = mockReq({
      params: { jobId: 'job-1' },
      body: { context: 'I already know they use React' },
    });
    const res = mockRes();
    vi.mocked(prisma.job.findUnique).mockResolvedValue(fakeJob as any);
    openaiCreateMock.mockResolvedValue({
      choices: [
        { message: { content: 'Structured research notes draft here' } },
      ],
    });
