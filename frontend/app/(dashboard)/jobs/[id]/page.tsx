...
export default function JobDetailPage({
  ...
  const [activeRewriteTarget, setActiveRewriteTarget] = useState<
    'resume' | 'coverLetter' | null
  >(null);

  // Save AI draft as a document linked to this job (S2-024)
  const [savingDoc, setSavingDoc] = useState<'resume' | 'coverLetter' | null>(
    null
  );
  ...
  useEffect(() => {
    async function fetchAll() {
      const [jobRes, interviewRes, followUpRes, timelineRes, docsRes] =
        await Promise.all([
      ...
      if (!jobRes || !jobRes.success) {
        ...
        setJob(jobRes.data);
        setDeadlineInput(
          jobRes.data.deadline ? formatDateTimeLocal(jobRes.data.deadline) : ''
        );
  ...
  async function applyStageChange(nextStage: string, override = false) {
    ...
    try {
      ...
      const timelineRes = await apiFetch(`/api/jobs/${id}/timeline`).catch(
        () => ({ data: [] })
      );
      if (timelineRes?.data) setTimeline(timelineRes.data);
    } catch {
  ...
  async function handleSaveInterview() {
    ...
    try {
      if (editingInterview) {
        ...
        setInterviews((prev) =>
          prev.map((i) => (i.id === editingInterview.id ? res.data : i))
        );
      } else {
  ...
  async function handleSaveFollowUp() {
    ...
    try {
      if (editingFollowUp) {
        ...
        setFollowUps((prev) =>
          prev.map((f) => (f.id === editingFollowUp.id ? res.data : f))
        );
      } else {
  ...
  async function handleToggleFollowUp(followUp: FollowUp) {
    try {
      ...
      setFollowUps((prev) =>
        prev.map((f) => (f.id === followUp.id ? res.data : f))
      );
      const timelineRes = await apiFetch(`/api/jobs/${id}/timeline`).catch(
        () => ({ data: [] })
      );
  ...
  function handleEditFollowUp(followUp: FollowUp) {
    ...
    setFollowUpForm({
      ...
      dueDate: formatDateTimeLocal(followUp.dueDate),
    });
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#0d1117] p-6 text-[#8b949e]">
        Loading...
      </div>
    );
  ...
  const isStale =
    getDaysSinceUpdate(job.updatedAt) >= 7 &&
    job.stage !== 'Rejected' &&
    !job.archivedAt;
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] p-6 text-[#e6edf3]">
      <div className="max-w-3xl mx-auto space-y-6">
        <Link
          href="/dashboard"
          className="text-sm text-[#2f81f4] hover:underline inline-block"
        >
          &larr; Back to dashboard
        </Link>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      setEditingMeta(false);
                      setDeadlineInput(
                        job.deadline ? formatDateTimeLocal(job.deadline) : ''
                      );
        ...
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6">
          ...
          {docSavedMessage && (
            <p className="text-xs text-[#3fb950] mb-3">{docSavedMessage}</p>
          )}
          {docError && (
            <p className="text-xs text-[#f85149] mb-3">{docError}</p>
          )}
          <button
            ...
            <div className="flex flex-col gap-2">
              ...
              <div>
                <button
                  ...
                  disabled={savingDoc === 'resume'}
                  className="text-xs px-3 py-1.5 bg-[#238636] text-white rounded hover:bg-[#2ea043] transition-colors disabled:opacity-50"
                >
          ...
          {coverLetterDraft && (
            <div className="flex flex-col gap-2 mt-4">
              ...
              <div>
                <button
                  onClick={() => handleSaveDocument('coverLetter')}
                  disabled={savingDoc === 'coverLetter'}
                  className="text-xs px-3 py-1.5 bg-[#238636] text-white rounded hover:bg-[#2ea043] transition-colors disabled:opacity-50"
                >
                  {savingDoc === 'coverLetter'
                    ? 'Saving...'
                    : 'Save Cover Letter'}
                </button>
              </div>
            ...
            <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6 mt-2">
              ...
              <div className="flex gap-2 mb-3">
                ...
                {resumeDraft && (
                  ...
                  >
                    {rewriting && activeRewriteTarget === 'resume'
                      ? 'Rewriting...'
                      : 'Rewrite Resume'}
                  </button>
                )}
                {coverLetterDraft && (
                  ...
                  >
                    {rewriting && activeRewriteTarget === 'coverLetter'
                      ? 'Rewriting...'
              ...
              {showComparison && originalDraft && (
                <div className="mt-4">
                  <p className="text-xs text-[#8b949e] mb-3">
                    Compare the original and rewritten version. Pick which one
                    to keep.
                  </p>
        ...
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6">
          ...
          {loadingDocs ? (
            ...
            <p className="text-xs text-[#8b949e]">
              No saved documents yet. Generate a draft above and click Save.
            </p>
          ) : (
              ...
              {interviews.map((interview) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="text-xs px-2 py-0.5 rounded bg-[#2d1f6e] text-[#bc8cff] mr-2">
                        {interview.roundType}
                      </span>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="text-xs text-[#f85149] hover:text-red-400 transition-colors"
                      >
                        Delete
                      </button>
                    </div>
                  ...
                  {interview.notes && (
                    <p className="text-xs text-[#8b949e] mt-2 whitespace-pre-wrap">
                      {interview.notes}
                    </p>
                  )}
                </div>
          ...
          {followUps.length === 0 && !showFollowUpForm ? (
            ...
            <div className="space-y-3">
              {followUps.map((f) => (
                <div
                  key={f.id}
                  className={`border rounded-lg p-3 flex justify-between items-start ${f.completed ? 'border-[#30363d] opacity-60' : 'border-[#30363d]'}`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p
                        className={`text-sm ${f.completed ? 'line-through text-[#8b949e]' : 'text-white'}`}
                      >
                        {f.title}
                      </p>
                      <p className="text-xs text-[#8b949e] mt-0.5">
      ...
      {showTransitionWarning && pendingStage && (
        ...
        >
          ...
          >
            ...
            >
              Moving from{' '}
              <strong style={{ color: '#e6edf3' }}>{job.stage}</strong> to{' '}
              <strong style={{ color: '#e6edf3' }}>{pendingStage}</strong> is
              not a standard forward transition.
            </p>
            <p
              style={{
                color: '#8b949e',
