...
export function JobDocumentLinks({ jobId }: { jobId: string }) {
  ...
  const [confirmReplace, setConfirmReplace] = useState<{ type: string; documentId: string; existingTitle: string } | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return
        if (libRes.success && Array.isArray(libRes.data)) setLibrary(libRes.data)
  ...
  async function performLink(type: string, documentId: string, confirmedReplace = false) {
    setSaving(true)
    try {
      ...
      if (status === 409) {
        const doc = library.find((d) => d.id === documentId)
        setConfirmReplace({ type, documentId, existingTitle: links[type]?.title ?? doc?.title ?? 'the current document' })
      }
    }
    setSaving(false)
  }
  ...
  async function handleUnlink(type: string) {
    ...
    try {
      await apiFetch(`/api/documents/jobs/${jobId}/link/${type}`, { method: 'DELETE' })
  ...
  return (
    <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6">
      <h2 className="text-sm font-semibold text-white mb-4">Linked Documents</h2>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {DOC_TYPES.map(({ value, label }) => {
            ...
            return (
              <div key={value} className="border border-[#30363d] rounded-lg p-3">
                <div className="flex justify-between items-center">
                  ...
                  {selecting !== value && (
                    <div className="flex gap-3">
                      ...
                      <button onClick={() => { setSelecting(value); setSelectedDocId('') }} className="text-xs text-[#2f81f4] hover:underline">
                        {linked ? 'Change' : 'Link'}
                      </button>
                    </div>
                  )}
                  ...
                  <div className="mt-3 space-y-2">
                    {availableForType.length === 0 ? (
                      <p className="text-xs text-[#8b949e]">
                        No {label.toLowerCase()}s in your library yet. Save one from a job&apos;s AI draft first.
                      </p>
                    ) : (
                      <select
                        value={selectedDocId}
                      ...
                      >
                        ...
                        {availableForType.map((d) => (
                          <option key={d.id} value={d.id}>{d.title} (v{d.versionNumber})</option>
                        ))}
                      </select>
                    )}
                    <div className="flex gap-2">
                      <button
                        onClick={() => selectedDocId && performLink(value, selectedDocId)}
                      ...
                      <button
                        onClick={() => { setSelecting(null); setSelectedDocId('') }}
      ...
      {confirmReplace && (
        ...
        >
          <div style={{ background: '#161b22', border: '1px solid #f85149', borderRadius: '8px', padding: '24px', maxWidth: '400px', width: '100%' }}>
            <h3 style={{ color: '#f85149', fontSize: '16px', fontWeight: 600, marginBottom: '8px' }}>Replace Linked Document</h3>
            <p style={{ color: '#8b949e', fontSize: '14px', marginBottom: '20px' }}>
              This job already has <strong style={{ color: '#e6edf3' }}>{confirmReplace.existingTitle}</strong> linked.
              Replacing it will not delete the document from your library.
            </p>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              <button
                type="button"
                onClick={() => setConfirmReplace(null)}
                style={{ padding: '8px 16px', border: '1px solid #30363d', background: 'transparent', color: '#8b949e', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={() => performLink(confirmReplace.type, confirmReplace.documentId, true)}
                style={{ padding: '8px 16px', background: '#f85149', border: 'none', color: '#ffffff', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}
              >
                Replace
              </button>
            </div>
